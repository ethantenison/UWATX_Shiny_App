---
title: "UWATX- 211 Explorer ReadMe & Data Cleaning File"
...
# General Data Notes
Most of the data was obtained using American Factfinder. The geography selected was the "5-Digit ZIP code Tabulation Area". If you click the "5-Digit Zip Code" option you will not find the same results. I recommend using the "Data_Cleaning.RMD" file to make alterations to the data. This file type will allow you to run one chuck of code at a time, and it will tell you where you are experiencing errors. This same RMD file is what I used to create this readme file.

## Updating additional years to the shiny app
The first thing you need to do is click the R Project file entitled "UWATX_Shiny_App". This ensure that you are using the right directory. Next you need to download the new files. Above each code chuck I've put the information you need to find them. ACS data should be downloaded and unzipped into the "ACS Data" folder, and the 211 data should be added to the "data" folder. The ACS data is consistently named the same way, however, the file name for the 211 data seems to change every year, so you need to make sure you input it correctly. 

Once the new files are downloaded, it's time to make code edits. Included in each code chuck is sections denoted by "#2016". All you have to do is copy and paste the entire section and simply change the dates througout. Make sure you change the ACS file names as well. For example, "./ACS_17_5YR_DP05_with_ann.csv" would need to be changed to "./ACS_18_5YR_DP05_with_ann.csv" when you add the data from 2018. In most of the code chucks there is also a section that binds the years together into one data table, such as #binding rows in the first chuck. Here all you need to do is include the new year. 

Whenever you make a change to one code chuck, if you want it to show up in the app, you need run the code for data_4_download followed by data_4_analysis before you go to the app.R script. 
...
demography <- bind_rows(demography_2016, demography_2017, demography_2018)

## Editing app.R 

The only major alterations you should be making to the code is changing the dates for the slider. Go to " max = as.Date" and change it to reflect the most recent year that is reflected in the data. 
...
The data for demographic information came from the "ACS DEMOGRAPHIC AND HOUSING ESTIMATES" table, id = DP05. Variables were selected to isolate the hispanic population in our sample. This was done so that the race percentages would add up to 100%.

```{r demography, warning=FALSE}
#2016

demography_2016<- read.csv("./ACS_16_5YR_DP05_with_ann.csv")
...
demography_2017<- read.csv("./ACS_17_5YR_DP05_with_ann.csv")
...
#saving data 
write_rds(demography, "./demographics.rds")
```

## Education 

The data for education information came from the ACS "EDUCATIONAL ATTAINMENT FOR THE POPULATION 25 YEARS AND OVER" table, id = B15003. Some of the columns were added together in order to create appropriate categories. For example, "high school diploma" and "GED" were added together to create "high school & GED". In addition, "associates degree" and "some college" were lumped together, and higher education was collapsed into one "bachelors plus" column. Finally, they were converted into percentages.  

```{r education attainment,warning=FALSE}
#2016
education_2016<- read.csv("./ACS_16_5YR_B15003_with_ann.csv")
...
#2017
education_2017<- read.csv("./ACS_17_5YR_B15003_with_ann.csv")
...
#save R object 
write_rds(education, "./education_attainment.rds")

```

## Health Insurance

The data for education information came from the ACS "Selected Economic Characteristics-5 year estimates" table, id =  DP03. Nothing was really changed besides adding a percentage sign. 


```{r health insurance, warning=FALSE}
#2016

insurance_2016<- read.csv("./ACS_16_5YR_DP03_with_ann.csv")
...
insurance_2017<- read.csv("./ACS_17_5YR_DP03_with_ann.csv")
...
#save r Object
write_rds(insurance, "./health_insurance.rds")
...
The data for housing information came from the ACS "SELECTED HOUSING CHARACTERISTICS-5 year estimate" table, id =  DP04. The values were unchanged. 

```{r housing units, warning=FALSE}
#2016
housing_units_2016<- read.csv("./ACS_16_5YR_DP04_with_ann.csv")
...
#2017
housing_units_2017<- read.csv("./ACS_17_5YR_DP04_with_ann.csv")
...
#save R object file 
write_rds(housing_units, "./total_housing_units.rds")
```


## Median Rent

The data for rent came from the ACS "Gross Median Rent - 5 year estimate" table, id =  B25064. These values remain unchagned. 

```{r median rent, warning=FALSE}
#2016

medianRent_2016<- read.csv("./ACS_16_5YR_B25064_with_ann.csv")
...
medianRent_2017<- read.csv("./ACS_17_5YR_B25064_with_ann.csv")
...
#save R object 
write_rds(medianRent, "./median_rent.rds")
...
The data for home values came from the ACS "SELECTED HOUSING CHARACTERISTICS-5 year estimate" table, id = DP04.

```{r median home value, warning=FALSE}

#2016
housing_median_price_2016<- read.csv("./ACS_16_5YR_DP04_with_ann.csv")
...
#2017
housing_median_price_2017<- read.csv("./ACS_17_5YR_DP04_with_ann.csv")
...
#save r object
write_rds(housing_median_price, "./median_housing_price.rds")
...
The data for education information came from the ACS "MEDIAN FAMILY INCOME IN THE PAST 12 MONTHS (IN 2017 INFLATION-ADJUSTED DOLLARS) BY FAMILY TYPE BY PRESENCE OF OWN CHILDREN UNDER 18 YEARS" table, id = B19126. The values are unchanged from the original. 


```{r median family income, warning=FALSE}
#2016

median_income_family_2016<- read.csv("./ACS_16_5YR_B19126_with_ann.csv")
...
median_income_family_2017<- read.csv("./ACS_17_5YR_B19126_with_ann.csv")
...
median_income_family <- median_income_family %>% mutate(median_monthly_family_income = (median_family_income)/12) %>% select(zipcode, median_family_income, median_monthly_family_income, date)

median_income_family$median_monthly_family_income<- format(median_income_family$median_monthly_family_income, digits=2, nsmall=0)

#saving data 
write_rds(median_income_family, "./median_family_income.rds")
...
The data for home values came from the ACS " POVERTY STATUS IN THE PAST 12 MONTHS-5 year estimate" table, id = S1701. The values remain unchanged. 

```{r percent below poverty, warning=FALSE}
#2016
poverty_status_2016 <- read.csv("./ACS_16_5YR_S1701_with_ann.csv")
...
#2017
poverty_status_2017 <- read.csv("./ACS_17_5YR_S1701_with_ann.csv")
...
#Save R object
        write_rds(poverty_status, "./percent_below_poverty.rds")
...
The data for 60 and above came from the ACS "AGE AND SEX" table, id = S0101. The values remain unchanged. 

```{r percent above 60, warning=FALSE}
 #2016
        over60_2016<- read.csv("./ACS_16_5YR_S0101_with_ann.csv")
...
#2017
        over60_2017<- read.csv("./ACS_17_5YR_S0101_with_ann.csv")
...
The data for under 6 came from the ACS " POPULATION UNDER 18 YEARS BY AGE" table, id = B09001, and from "TOTAL POPULATION", id = B01003. You need both of these data sets in order for this to run. Using "TOTAL POPULATION", a column was added denoting the percentage of children under 6 in each zip code.  

```{R under 6, warning=FALSE}

#2016
under6_2016<- read.csv("./ACS_16_5YR_B09001_with_ann.csv") 
total_pop_2016 <- read.csv("./ACS_16_5YR_B01003_with_ann.csv") 
...
#2017
under6_2017<- read.csv("./ACS_17_5YR_B09001_with_ann.csv") 
total_pop_2017 <- read.csv("./ACS_17_5YR_B01003_with_ann.csv") 
...
#save R object 
write_rds(under6, "./under6.rds")

```

## Treemap Master 

The UW icarol file was used to ocreate the treemap master. This document shows every single 211 referral along with year,zip, age, gender, language, military status, and the name of the need. Calls that were transfered or disconnected were removed. Age has been changed to reflect discrete categories, such as 13-18 year olds and 60 and up. In addition, veterans who did not report military status was changed to "undetermined".

```{R treemap master, warning=FALSE}

#2016
icarol_2016 <- openxlsx::read.xlsx("./2016 CNT iCarol Export.xlsx", sheet = 1, startRow = 1, colNames = TRUE)
...
icarol_2016 <- filter(icarol_2016, age != "N/A")
icarol_2016 <- filter(icarol_2016, age != "Refused")
icarol_2016$age <- na.omit(icarol_2016$age)
...
#2017
icarol_2017 <- openxlsx::read.xlsx("./2017 iCarol Export 1.3.18.xlsx", 
...
icarol_2017 <- filter(icarol_2017, age != "N/A")
icarol_2017 <- filter(icarol_2017, age != "Refused")
icarol_2017$age <- na.omit(icarol_2017$age)

for (i in 1:length(icarol_2017$age)){
                  if(icarol_2017$age[i] >= 13 & icarol_2017$age[i] < 18){        
...
write_rds(treemap_master, "./treemap_master.rds")

```

## Needs Zip treemap 

The UW icarol file was used to ocreate the treemap master.This r objects shows the sum of every type of referral by year, zipcode, and needs category. Calls that were transfered or disconnected were removed. Then the data was grouped by zip code and the need name. Finally, the total calls per need were tallied. NA values were removed and any need that had less than 5 calls was as well. 
```{R needs zip treemap, warning=FALSE}


#2016
icarol_2016 <- openxlsx::read.xlsx("./2016 CNT iCarol Export.xlsx", 
...
#2017
icarol_2017 <- openxlsx::read.xlsx("./2017 iCarol Export 1.3.18.xlsx", sheet = 2, startRow = 1, colNames = TRUE)
...
write_rds(needs_zip_treemap, "./needs_zip_treemap.rds")
...
The UW icarol file was used to create the total 211 calls table. In addition, the data for housing units was used to calculate the total calls per 100 households. Calls that were transfered or disconnected were removed, and the data was groups by zipcode and year. The number of digits was also reduced to two. 

```{R total 211 calls, warning=FALSE}
...
#Save R object 
write_rds(total_211_calls, "./total_211_calls.rds")

```

## Data that can be downloaded 

This r object uses most of the tables created above. The data was joined using the zip code and then the date. In the Shiny app, viewers can download the selected data that appears on the map. 

```{R data4download, warning=FALSE}
...
total_211_calls <- read_rds("./total_211_calls.rds")
...
write_rds(data_4_download, "./data_4_download.rds")
...
This R object used the data_4_download object to sum up the total referrals per zip code, irrespective of the need name. This information appears in the static section of the Shiny app.
```{r total 211 referrals, warning=FALSE}
data <- read_rds("./data_4_download.rds")

summ211data <- data %>% group_by(Year) %>% summarise(total_referrals = sum(total_calls))


write_rds(data_4_download, "./summ211.rds")
```

## Data for use in the app 

This R object is used to project data on the map. First the data_4_download object is used, along with shape file for the zipcode geometry. The table is converted from wide to long format, in which each variable is assigned into the "measure" column. Another column is created called "displayed measure", which converts the measure label into a user friendly format. Then another column called "category", splits all the measures into categories that will be used in the drop down menu. 

Finally the spatial data is joined using the zip codes. The shapefile was shortened to only contain the important zipcodes. Unfortunatley the original file is too large to upload to Shiny because it contains all Texas zipcodes.You can find the large shape file here : https://catalog.data.gov/dataset/tiger-line-shapefile-2015-2010-nation-u-s-2010-census-5-digit-zip-code-tabulation-area-zcta5-na

Click on the Shape file zip download. 

```{R data4analysis, warning=FALSE}
#reading the existing data 
data <- read_rds("./data_4_download.rds")

convert_columns <- colnames(data[3:25]) #If you add a variable you need to change the number of columns and you also need to change new.row.names

data_4_analysis <- reshape(data, idvar = c("Year", "zipcode"), varying = c(convert_columns),
                          v.name = c("value"), times = c(convert_columns), new.row.names = 1:2438,
                           direction = "long") 
...
data_4_analysis <- mutate(data_4_analysis, displayed_measure = recode(data_4_analysis$measure, 
                          ...
                          "HS Diploma or GED (%)", percent_associates_somecollege = "Associates Degree or some College (%)",
                          ...
                          = "Median Family Income", median_monthly_family_income = "Median Monthly Family Income", housing_median_price = "Median Household Value", 
...
data_4_analysis <- mutate(data_4_analysis, category = recode(data_4_analysis$measure, 
                          ...
                          = "Financial", median_monthly_family_income = "Income & Poverty", housing_median_price = "Households", 
                          estimate_median_gross_rent = "Income & Poverty", percent_over60 = "Age",
                          children_under6 = "Total Age", percent_below_poverty_level = 
...
library(sf)
zip <- read_sf(dsn = ".", layer = "zip_shapefile")
...
#Save R object 
write_rds(data_4_analysis, "./data_4_analysis.rds")
