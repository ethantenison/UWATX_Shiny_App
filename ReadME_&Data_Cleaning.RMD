...
The first thing you need to do is click the R Project file entitled "UWATX_Shiny_App". This ensure that you are using the right directory. Next you need to download the new files. Above each code chuck I've put the information you need to find them. ACS data should be downloaded and unzipped into the "ACS Data" folder, and the 211 data should be added to the "211 Data" folder. The ACS data is consistently named the same way, however, the file name for the 211 data seems to change every year, so you need to make sure you input it correctly. 

Once the new files are downloaded, it's time to make code edits. Included in each code chuck is sections denoted by "#2016". All you have to do is copy and paste the entire section and simply change the dates througout. Make sure you change the ACS file names as well. For example, "./ACS_Data/ACS_17_5YR_DP05_with_ann.csv" would need to be changed to "./ACS_Data/ACS_18_5YR_DP05_with_ann.csv" when you add the data from 2018. In most of the code chucks there is also a section that binds the years together into one data table, such as #binding rows in the first chuck. Here all you need to do is include the new year. 
...
Finally the spatial data is joined using the zip codes. You can find the shape file here : https://catalog.data.gov/dataset/tiger-line-shapefile-2015-2010-nation-u-s-2010-census-5-digit-zip-code-tabulation-area-zcta5-na
